...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        if (!ProcessNewBlock(GetParams(), shared_pblock, true, nullptr, shared_pblock->GetIndexHash()))
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated paladeum to.\n"
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when paladeumd is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "    https://github.com/paladeum/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/paladeum/bips/blob/master/bip-0023.mediawiki\n"
            "    https://github.com/paladeum/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
            "    https://github.com/paladeum/bips/blob/master/bip-0145.mediawiki\n"
    ...
    if (!request.params[0].isNull())
    {
        ...
        if (strMode == "proposal")
        {
            ...
            if (block.hashPrevBlock != pindexPrev->GetIndexHash())
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Paladeum is not connected!");

    if (IsInitialBlockDownload() && !gArgs.GetBoolArg("-bypassdownload", false))
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Paladeum is downloading blocks...");
    ...
    if (!lpval.isNull())
    {
        ...
        else
        {
            ...
            hashWatchedChain = chainActive.Tip()->GetIndexHash();
        ...
        LEAVE_CRITICAL_SECTION(cs_main);
        {
            ...
            while (chainActive.Tip()->GetIndexHash() == hashWatchedChain && IsRPCRunning())
            {
...
//    const struct VBDeploymentInfo& segwit_info = VersionBitsDeploymentInfo[Consensus::DEPLOYMENT_SEGWIT];
    ...
    result.push_back(Pair("longpollid", chainActive.Tip()->GetIndexHash().GetHex() + i64tostr(nTransactionsUpdatedLast)));
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("estimatefee")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "estimatefee is deprecated and will be fully removed in v0.17. "
            "To use estimatefee in v0.16, restart paladeumd with -deprecatedrpc=estimatefee.\n"
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    int numCores = GenerateCoins(fGenerate, nGenProcLimit, GetParams());
