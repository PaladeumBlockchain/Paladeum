...
{
    return AreTokensDeployed() ? "" : "\nTHIS COMMAND IS NOT YET ACTIVE!\nhttps://github.com/PaladeumProject/rips/blob/master/rip-0002.mediawiki\n";
}
    ...
    if (request.params.size() > 2) {
        ...
        if (!change_address.empty()) {
           ...
           if (!IsValidDestination(change_dest)) {
               throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Paladeum change address: ") + change_address);
           }
        }
    }
...
UniValue issue(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreTokensDeployed() || request.params.size() < 1 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "4. \"change_address\"        (string), optional, default=\"\"), address the the paladeum change will be sent to, if it is empty, change address will be generated for you\n"
        ...
        if (!change_address.empty()) {
            ...
            if (!IsValidDestination(destination)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                                   std::string("Invalid Change Address: Invalid Paladeum address: ") + change_address);
            }
        }
    }
...
UniValue issueunique(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreTokensDeployed() || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "5. \"change_address\"        (string, optional, default=\"\"), address the the paladeum change will be sent to, if it is empty, change address will be generated for you\n"
    ...
    if (!changeAddress.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid Change Address: Invalid Paladeum address: ") + changeAddress);
        }
    }
...
UniValue listtokenbalancesbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !AreTokensDeployed() || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"                  (string, required) a paladeum address\n"
...
UniValue transfer(const JSONRPCRequest& request)
{
    ...
    if (!IsValidDestination(to_dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Paladeum address: ") + to_address);
    }
    ...
    std::string paladeum_change_address = "";
    if (request.params.size() > 7) {
        paladeum_change_address = request.params[7].get_str();
    }
    ...
    CCoinControl ctrl;
    ctrl.destChange = paladeum_change_dest;
...
UniValue transferfromaddresses(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreTokensDeployed() || request.params.size() < 4 || request.params.size() > 10)
        throw std::runtime_error(
            "transferfromaddresses \"token_name\" [\"from_addresses\"] qty \"to_address\" timelock \"message\" \"token_message\" expire_time \"paladeum_change_address\" \"token_change_address\"\n"
            + TokenActivationWarning() +
    ...
    std::string paladeum_change_address = "";
    if (request.params.size() > 8) {
        paladeum_change_address = request.params[8].get_str();
    }
...
UniValue transferfromaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreTokensDeployed() || request.params.size() < 4 || request.params.size() > 10)
        throw std::runtime_error(
                "transferfromaddress \"token_name\" \"from_address\" qty \"to_address\" timelock \"message\" \"token_message\" expire_time \"paladeum_change_address\" \"token_change_address\"\n"
                + TokenActivationWarning() +
...
UniValue sweep(const JSONRPCRequest& request)
{
    ...
    // TODO: Should this be replaced with a call to the api.paladeumcoin.network?
    ...
    // }
    UniValue unspent;
    UniValue unspent_paladeum;
    UniValue unspent_our_paladeum;
    {
    ...
    UniValue created_transaction;
    {
        ...
        if (fee_left != 0) {
            if (
                ...
                for (size_t i = 0; i != unspent_paladeum.size() && fee_left != 0; ++i) {
                    UniValue current_input = UniValue(UniValue::VOBJ);

                    const UniValue& current = unspent_paladeum[i];
                    CAmount curr_amount;
            ...
            if (fee_left != 0) {
                // Add as many UTXOs as needed until we either run out or successfully
                //   fund the transaction
                for (size_t i = 0; i != unspent_our_paladeum.size() && fee_left != 0; ++i) {
                    UniValue current_input = UniValue(UniValue::VOBJ);

                    const UniValue& current = unspent_our_paladeum[i];
...
UniValue issuerestrictedtoken(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedTokensDeployed() || request.params.size() < 4 || request.params.size() > 10)
        throw std::runtime_error(
                ...
                "5. \"change_address\"        (string, optional, default=\"\") address that the paladeum change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue reissuerestrictedtoken(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedTokensDeployed() || request.params.size() < 3 || request.params.size() > 10)
        throw std::runtime_error(
                ...
                "6. \"change_address\"        (string, optional, default=\"\") address that the paladeum change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue transferqualifier(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() > 3) {
        ...
        CTxDestination change_dest = DecodeDestination(change_address);
        if (!IsValidDestination(change_dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Paladeum address: ") + change_address);
        }
    }
...
#ifdef ENABLE_WALLET
    { "tokens",   "transferfromaddress",        &transferfromaddress,        {"token_name", "from_address", "qty", "to_address", "timelock", "message", "token_message", "expire_time", "paladeum_change_address", "token_change_address"}},
    { "tokens",   "transferfromaddresses",      &transferfromaddresses,      {"token_name", "from_addresses", "qty", "to_address", "timelock", "message", "token_message", "expire_time", "paladeum_change_address", "token_change_address"}},
