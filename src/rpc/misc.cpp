...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total Paladeum balance of the wallet\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given paladeum address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The paladeum address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The paladeum address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"stakingaddress\" : \"paladeumaddress\", (string) The staking address part of a offline staking address\n"
            "  \"spendingaddress\" : \"paladeumaddress\", (string) The spending address part of a offline staking address\n"
    ...
    if (isValid)
    {
        ...
        CPaladeumAddress address(request.params[0].get_str());
        if (address.IsOfflineStakingAddress(GetParams())) {
            CPaladeumAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CPaladeumAddress spendingAddress;
...
#ifdef ENABLE_WALLET
        // Case 1: Paladeum address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are paladeum addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) paladeum address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The paladeum address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "paladeum-cli and the GUI. There is no server-side difference."
        );
...
{
    if (type == 2) {
        address = CPaladeumAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CPaladeumAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  {\n"
            "    \"address\"  (string) The base58check encoded address\n"
            "    \"tokenName\"  (string) The name of the associated token (PLB for Paladeum)\n"
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "\"address\"         (string, required) The paladeum address.\n"
...
UniValue getaddressdeltas(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1 || !request.params[0].isObject())
        throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"tokenName\"  (string) The token associated with the deltas (PLB for Paladeum)\n"
            "    \"satoshis\"  (number) The difference of satoshis\n"
            "    \"txid\"  (string) The related txid\n"
            "    \"index\"  (number) The related input or output index\n"
            "    \"height\"  (number) The block height\n"
