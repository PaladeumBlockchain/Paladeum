...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total Akilacoin balance of the wallet\n"
...
public:
    ...
    UniValue operator()(const CKeyID &keyID) const {
        ...
        obj.push_back(Pair("isofflinestaking", false));
    ...
    UniValue operator()(const std::pair<CKeyID, CKeyID> &keyID) const {
        ...
        CPubKey vchPubKey;
        obj.push_back(Pair("isscript", false));
        obj.push_back(Pair("isofflinestaking", true));
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given akila address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The akila address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The akila address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"stakingaddress\" : \"akilaaddress\", (string) The staking address part of a offline staking address\n"
            "  \"spendingaddress\" : \"akilaaddress\", (string) The spending address part of a offline staking address\n"
            "  \"ismine\" : true|false,        (boolean) If the address is yours or not\n"
            "  \"iswatchonly\" : true|false,   (boolean) If the address is watchonly\n"
            "  \"isscript\" : true|false,      (boolean) If the key is a script\n"
            "  \"isofflinestaking\" : true|false,        (boolean) If the address is a offline staking address or not\n"
    ...
    if (isValid)
    {
        ...
        CAkilaAddress address(request.params[0].get_str());
        if (address.IsOfflineStakingAddress(GetParams())) {
            CAkilaAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CAkilaAddress spendingAddress;
        ...
        ret.push_back(Pair("isstakable", (mine & ISMINE_STAKABLE || (mine & ISMINE_SPENDABLE &&
                     !address.IsOfflineStakingAddress(GetParams()))) ? true : false));
...
#ifdef ENABLE_WALLET
        // Case 1: Akila address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are akila addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) akila address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The akila address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "akila-cli and the GUI. There is no server-side difference."
        );
...
{
    if (type == 2) {
        address = CAkilaAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CAkilaAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  {\n"
            "    \"address\"  (string) The base58check encoded address\n"
            "    \"tokenName\"  (string) The name of the associated token (AKILA for Akilacoin)\n"
    ...
    if (includeTokens) {
        ...
        if (!mempool.getAddressIndex(addresses, AKILA, indexes)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
        }
    }
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "\"address\"         (string, required) The akila address.\n"
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++) {
        ...
        int64_t timeLock = (tokenName == AKILA) ? it->first.timeLock : nTimeLock;
...
UniValue getaddressdeltas(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1 || !request.params[0].isObject())
        throw std::runtime_error(
            ...
            "  \"tokenName\"   (string, optional) Get deltas for a particular token instead of AKILA.\n"
            "}\n"
            "\nResult:\n"
            "[\n"
            "  {\n"
            "    \"tokenName\"  (string) The token associated with the deltas (AKILA for Akilacoin)\n"
            "    \"satoshis\"  (number) The difference of satoshis\n"
            "    \"txid\"  (string) The related txid\n"
            "    \"index\"  (number) The related input or output index\n"
            "    \"height\"  (number) The block height\n"
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    ...
    if (includeTokens) {
        ...
        for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
            if (!GetAddressIndex((*it).first, (*it).second, AKILA, addressIndex)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
            }
        }
...
UniValue getaddresstxids(const JSONRPCRequest& request)
{
    ...
    for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
        if (includeTokens) {
            ...
            if (start > 0 && end > 0) {
                if (!GetAddressIndex((*it).first, (*it).second, AKILA, addressIndex, start, end)) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
                }
            } else {
                if (!GetAddressIndex((*it).first, (*it).second, AKILA, addressIndex)) {
