...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total Yonacoin balance of the wallet\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given yona address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The yona address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The yona address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"stakingaddress\" : \"yonaaddress\", (string) The staking address part of a offline staking address\n"
            "  \"spendingaddress\" : \"yonaaddress\", (string) The spending address part of a offline staking address\n"
    ...
    if (isValid)
    {
        ...
        CYonaAddress address(request.params[0].get_str());
        if (address.IsOfflineStakingAddress(GetParams())) {
            CYonaAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CYonaAddress spendingAddress;
...
#ifdef ENABLE_WALLET
        // Case 1: Yona address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are yona addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) yona address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The yona address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "yona-cli and the GUI. There is no server-side difference."
        );
...
{
    if (type == 2) {
        address = CYonaAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CYonaAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  {\n"
            "    \"address\"  (string) The base58check encoded address\n"
            "    \"tokenName\"  (string) The name of the associated token (YONA for Yonacoin)\n"
    ...
    if (includeTokens) {
        ...
        if (!mempool.getAddressIndex(addresses, YONA, indexes)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
        }
    }
...
UniValue getaddressdeltas(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1 || !request.params[0].isObject())
        throw std::runtime_error(
            ...
            "  \"tokenName\"   (string, optional) Get deltas for a particular token instead of YONA.\n"
            "}\n"
            "\nResult:\n"
            "[\n"
            "  {\n"
            "    \"tokenName\"  (string) The token associated with the deltas (YONA for Yonacoin)\n"
            "    \"satoshis\"  (number) The difference of satoshis\n"
            "    \"txid\"  (string) The related txid\n"
            "    \"index\"  (number) The related input or output index\n"
            "    \"height\"  (number) The block height\n"
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    ...
    if (includeTokens) {
        ...
        for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
            if (!GetAddressIndex((*it).first, (*it).second, YONA, addressIndex)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
            }
        }
...
UniValue getaddresstxids(const JSONRPCRequest& request)
{
    ...
    for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
        if (includeTokens) {
            ...
            if (start > 0 && end > 0) {
                if (!GetAddressIndex((*it).first, (*it).second, YONA, addressIndex, start, end)) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
                }
            } else {
                if (!GetAddressIndex((*it).first, (*it).second, YONA, addressIndex)) {
