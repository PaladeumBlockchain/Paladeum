...
UniValue issueunique(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "Five (5) YONA will be burned for each asset created.\n"
...
UniValue transferfromaddresses(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
            "transferfromaddresses \"asset_name\" [\"from_addresses\"] qty \"to_address\" \"message\" expire_time \"yona_change_address\" \"asset_change_address\"\n"
            + AssetActivationWarning() +
...
UniValue transferfromaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
                "transferfromaddress \"asset_name\" \"from_address\" qty \"to_address\" \"message\" expire_time \"yona_change_address\" \"asset_change_address\"\n"
                + AssetActivationWarning() +
 ...
 * YONA, but can be limited to either all of the YONA or one asset type by passing
...
UniValue sweep(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() > 2 || request.params.size() < 1)
        throw std::runtime_error(
                "sweep \"privkey\" ( \"asset_name\" | \"YONA\" ) \n"
                + AssetActivationWarning() +
                "\nCreates a transaction to transfer all YONA, and all Assets from a given address -- with only the private key as input.\n"
                "\nDefault to funding from YONA held in the address, fallback to using YONA held in wallet for transaction fee."
                "\nDefault to sweeping all assets, but can also all with YONA to sweep only YONA, or to sweep only one asset."
                ...
                "2. \"asset_name\"            (string, optional, default=\"\") name of the asset to sweep or YONA"
    ...
    if (swept_balance + our_balance < DEFAULT_MIN_RELAY_TX_FEE) {
        throw JSONRPCError(
            ...
            tfm::format(
                "Please add YONA to address '%s' to be able to sweep asset '%s'",
                addr_str,
                asset_name
            )
        );
    ...
    UniValue created_transaction;
    {
        ...
        for (size_t i = 0; i != unspent.size(); ++i) {
            ...
            if (fee_left != 0 && curr_asset_name == YONA) {
        ...
        if (fee_left != 0) {
            if (
                (asset_name != YONA && asset_name != "") && // We haven't already considered YONA in our sweep above
        ...
        for (const auto& it : asset_totals) {
            ...
            if (curr_asset_name == YONA) continue;

            curr_transfer.pushKV(curr_asset_name, ValueFromAmount(curr_amount));
        }

        // Add the YONA output, if available
        if (asset_totals.find(YONA) != asset_totals.end()) {
            CAmount yona_amount = asset_totals[YONA] - fee_paid_by_us;
...
#ifdef ENABLE_WALLET
    { "assets",   "transferfromaddress",        &transferfromaddress,        {"asset_name", "from_address", "qty", "to_address", "message", "expire_time", "yona_change_address", "asset_change_address"}},
    { "assets",   "transferfromaddresses",      &transferfromaddresses,      {"asset_name", "from_addresses", "qty", "to_address", "message", "expire_time", "yona_change_address", "asset_change_address"}},
