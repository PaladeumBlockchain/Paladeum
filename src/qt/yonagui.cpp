...
const std::string YonaGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
void YonaGUI::createActions()
{
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Yona addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Yona addresses"));
    ...
    openAction->setStatusTip(tr("Open a yona: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Yona command-line options").arg(tr(PACKAGE_NAME)));
...
void YonaGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool YonaGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool YonaGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void YonaGUI::setWalletActionsEnabled(bool enabled)
{
...
void YonaGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void YonaGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void YonaGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void YonaGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void YonaGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void YonaGUI::openClicked()
{
...
void YonaGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void YonaGUI::gotoHistoryPage()
{
...
void YonaGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void YonaGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void YonaGUI::gotoVerifyMessageTab(QString addr)
{
...
void YonaGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Yona network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void YonaGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void YonaGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void YonaGUI::updateHeadersSyncProgressLabel()
{
...
void YonaGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void YonaGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Yona"); // default title
...
void YonaGUI::changeEvent(QEvent *e)
{
...
void YonaGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void YonaGUI::showEvent(QShowEvent *event)
{
...
void YonaGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void YonaGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool YonaGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool YonaGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void YonaGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void YonaGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void YonaGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void YonaGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void YonaGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMnemonic(YonaGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void YonaGUI::subscribeToCoreSignals()
{
...
void YonaGUI::unsubscribeFromCoreSignals()
{
...
void YonaGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void YonaGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<YonaUnits::Unit> units = YonaUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const YonaUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(YonaUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(YonaUnits::name(unit)));
    #endif
    }
...
void UnitDisplayStatusBarControl::createContextMenu(const PlatformStyle *platformStyle)
{
    ...
    {
        QAction *menuAction = new QAction(QString(YonaUnits::name(u)), this);
...
{
    setText(YonaUnits::name(newUnits));
}
