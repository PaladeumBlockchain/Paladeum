...
const std::string AkilaGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString AkilaGUI::DEFAULT_WALLET = "~Default";

static bool ThreadSafeMessageBox(AkilaGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

double GetPoSKernelPS();

AkilaGUI::AkilaGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void AkilaGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Akila address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and akila: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Akila addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Akila addresses"));
    ...
    openAction->setStatusTip(tr("Open a akila: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Akila command-line options").arg(tr(PACKAGE_NAME)));
...
void AkilaGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        QPixmap logo_pixmap(":/icons/akila_white");
...
void AkilaGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool AkilaGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool AkilaGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void AkilaGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AkilaGUI::setWalletActionsEnabled(bool enabled)
{
...
void AkilaGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void AkilaGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void AkilaGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AkilaGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void AkilaGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void AkilaGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void AkilaGUI::openClicked()
{
...
void AkilaGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AkilaGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AkilaGUI::gotoOfflinePage()
{
    offlineAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOfflinePage();
}

void AkilaGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AkilaGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AkilaGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AkilaGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** TOKENS START */
void AkilaGUI::gotoTokensOverviewPage()
{
...
void AkilaGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Akila network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void AkilaGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AkilaGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void AkilaGUI::updateHeadersSyncProgressLabel()
{
...
void AkilaGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void AkilaGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Akila"); // default title
...
void AkilaGUI::changeEvent(QEvent *e)
{
...
void AkilaGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AkilaGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AkilaGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& tokenName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (tokenName == "AKILA")
        msg += tr("Amount: %1\n").arg(AkilaUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(AkilaUnits::formatWithCustomName(tokenName, amount, MAX_TOKEN_UNITS, true));
...
void AkilaGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AkilaGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AkilaGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AkilaGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AkilaGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void AkilaGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AkilaGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AkilaGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AkilaGUI::updateStakingIcon()
{
    if (walletFrame) {
...
void AkilaGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void AkilaGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void AkilaGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(AkilaGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void AkilaGUI::subscribeToCoreSignals()
{
...
void AkilaGUI::unsubscribeFromCoreSignals()
{
...
void AkilaGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void AkilaGUI::toggleStakingActive()
{
...
void AkilaGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<AkilaUnits::Unit> units = AkilaUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AkilaUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(AkilaUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(AkilaUnits::name(unit)));
    #endif
    }
...
void UnitDisplayStatusBarControl::createContextMenu(const PlatformStyle *platformStyle)
{
    ...
    {
        QAction *menuAction = new QAction(QString(AkilaUnits::name(u)), this);
...
{
    setText(AkilaUnits::name(newUnits));
}
