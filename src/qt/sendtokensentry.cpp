    ...
    platformStyle(_platformStyle)
{
    ...
    // normal paladeum address field
    GUIUtil::setupAddressWidget(ui->payTo, this);
    // just a label for displaying paladeum address(es)
...
bool SendTokensEntry::validate()
{
    ...
    if (!ui->memoBox->text().isEmpty()) {
        ...
        std::string error = "";
        if(!CheckEncoded(DecodeTokenData(ui->memoBox->text().toStdString()), error)) {
            ui->memoBox->setStyleSheet(STYLE_INVALID);
            retval = false;
        }

    }
    std::string tokenName = ui->tokenSelectionBox->currentText().toStdString();
    if (IsTokenNameAnRestricted(tokenName)) {
        if (ptokens) {
            if (ptokens->CheckForGlobalRestriction(tokenName)) {
                ui->tokenSelectionBox->lineEdit()->setStyleSheet(STYLE_INVALID);
                ui->messageTextLabel->show();
                ui->messageTextLabel->setText(tr("This restricted token has been frozen globally. No transfers can be sent on the network."));
                retval = false;
            }

            CNullTokenTxVerifierString verifier;
            if (ptokens->GetTokenVerifierStringIfExists(tokenName, verifier)) {
                std::string strError = "";
                ErrorReport report;
                if (!ContextualCheckVerifierString(ptokens, verifier.verifier_string,ui->payTo->text().toStdString(), strError, &report)) {
                    ui->payTo->setValid(false);
                    ui->messageTextLabel->show();
...
QWidget *SendTokensEntry::setupTabChain(QWidget *prev)
{
    ...
    QWidget::setTabOrder(ui->deleteButton, ui->payTokenAmount);
    QWidget::setTabOrder(ui->payTokenAmount, ui->memoBox);
    return ui->memoBox;
}
...
//            ui->payTokenAmount->setDisabled(false);
    ...
    // Check to see if the token selected is an messenger token
    bool fIsMessengerToken = false;
    if (IsTokenNameAnMsgChannel(name.toStdString())) {
        fIsMessengerToken = true;
    }
    ...
    // If it is not an ownership token unlock the amount
    if (!fIsOwnerToken) {
        ui->payTokenAmount->setUnit(token.units);
        ui->payTokenAmount->setSingleStep(1);
        ui->payTokenAmount->setDisabled(false);
        ui->payTokenAmount->setValue(0);
    }
    // If it is messanger channel set amount to 1 and keep locked.
    if (fIsMessengerToken) {
        ui->payTokenAmount->setUnit(token.units);
        ui->payTokenAmount->setDisabled(true);
        ui->payTokenAmount->setValue(1);
    }
}
...
bool SendTokensEntry::eventFilter(QObject *object, QEvent *event)
{
