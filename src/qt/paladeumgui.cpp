...
const std::string PaladeumGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString PaladeumGUI::DEFAULT_WALLET = "~Default";

static bool ThreadSafeMessageBox(PaladeumGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

double GetPoSKernelPS();

PaladeumGUI::PaladeumGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...

{
    ...
    {
        progressBar->setStyleSheet("QProgressBar { color: #ffffff; background-color: #e8e8e8; border: 1px solid grey; border-radius: 7px; padding: 1px; text-align: center; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 #1c57b3, stop: 1 #154287); border-radius: 5px; margin: 0px; }");
    }
...
void PaladeumGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Paladeum address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and paladeum: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Paladeum addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Paladeum addresses"));
    ...
    openAction->setStatusTip(tr("Open a paladeum: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Paladeum command-line options").arg(tr(PACKAGE_NAME)));
...
void PaladeumGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void PaladeumGUI::createToolBars()
{
    if(walletFrame)
    {
        /** TOKENS START */
        ...
        QPixmap logo_pixmap(":/icons/paladeum_white");
...
void PaladeumGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool PaladeumGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool PaladeumGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void PaladeumGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void PaladeumGUI::setWalletActionsEnabled(bool enabled)
{
...
void PaladeumGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void PaladeumGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void PaladeumGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void PaladeumGUI::optionsClicked()
{
...
void PaladeumGUI::showDebugWindow()
{
...
void PaladeumGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void PaladeumGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void PaladeumGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void PaladeumGUI::openClicked()
{
...
void PaladeumGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void PaladeumGUI::gotoHistoryPage()
{
...
void PaladeumGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void PaladeumGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void PaladeumGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** TOKENS START */
void PaladeumGUI::gotoTokensOverviewPage()
{
...
void PaladeumGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Paladeum network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void PaladeumGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void PaladeumGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void PaladeumGUI::updateHeadersSyncProgressLabel()
{
    int64_t headersTipTime = clientModel->getHeaderTipTime();
    int headersTipHeight = clientModel->getHeaderTipHeight();
    int estHeadersLeft = (GetTime() - headersTipTime) / GetParams().GetConsensus().nTargetSpacing;
    if (estHeadersLeft > HEADER_HEIGHT_DELTA_SYNC)
        progressBarLabel->setText(tr("Syncing Headers (%1%)...").arg(QString::number(100.0 / (headersTipHeight+estHeadersLeft)*headersTipHeight, 'f', 1)));
}

void PaladeumGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void PaladeumGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Paladeum"); // default title
...
void PaladeumGUI::changeEvent(QEvent *e)
{
...
void PaladeumGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void PaladeumGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void PaladeumGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& tokenName)
{
...
void PaladeumGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void PaladeumGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool PaladeumGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool PaladeumGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void PaladeumGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void PaladeumGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void PaladeumGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void PaladeumGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void PaladeumGUI::updateStakingIcon()
{
    if (walletFrame) {
...
void PaladeumGUI::detectShutdown()
{
...
void PaladeumGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void PaladeumGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void PaladeumGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(PaladeumGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void PaladeumGUI::subscribeToCoreSignals()
{
...
void PaladeumGUI::unsubscribeFromCoreSignals()
{
...
void PaladeumGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void PaladeumGUI::toggleStakingActive()
{
...
void PaladeumGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<PaladeumUnits::Unit> units = PaladeumUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const PaladeumUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(PaladeumUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(PaladeumUnits::name(unit)));
    #endif
    }
...
void UnitDisplayStatusBarControl::createContextMenu(const PlatformStyle *platformStyle)
{
    ...
    {
        QAction *menuAction = new QAction(QString(PaladeumUnits::name(u)), this);
...
{
    setText(PaladeumUnits::name(newUnits));
}
