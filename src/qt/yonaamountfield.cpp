...
public:
    ...
    void stepBy(int steps)
    {
        ...
        val = qMin(qMax(val, CAmount(0)), YonaUnits::maxMoney());
        setValue(val);
    }
...
private:
    ...
    CAmount parse(const QString &text, bool *valid_out=0) const
    {
        ...
        if(valid)
        {
            if(val < 0 || val > YonaUnits::maxMoney())
                valid = false;
        }
        if(valid_out)
            *valid_out = valid;
...
#include "yonaamountfield.moc"

YonaAmountField::YonaAmountField(QWidget *parent) :
    QWidget(parent),
    amount(0)
{
    ...
    unit->setModel(new YonaUnits(this));
...
void YonaAmountField::clear()
{
    amount->clear();
    unit->setCurrentIndex(0);
}

void YonaAmountField::setEnabled(bool fEnabled)
{
    amount->setEnabled(fEnabled);
    unit->setEnabled(fEnabled);
}

bool YonaAmountField::validate()
{
    bool valid = false;
    value(&valid);
    setValid(valid);
...
void YonaAmountField::setValid(bool valid)
{
    if (valid) {
...
bool YonaAmountField::eventFilter(QObject *object, QEvent *event)
{
...
QWidget *YonaAmountField::setupTabChain(QWidget *prev)
{
...
CAmount YonaAmountField::value(bool *valid_out) const
{
    return amount->value(valid_out);
}

void YonaAmountField::setValue(const CAmount& value)
{
    amount->setValue(value);
}

void YonaAmountField::setReadOnly(bool fReadOnly)
{
    amount->setReadOnly(fReadOnly);
}

void YonaAmountField::unitChanged(int idx)
{
    // Use description tooltip for current unit for the combobox
    unit->setToolTip(unit->itemData(idx, Qt::ToolTipRole).toString());

    // Determine new unit ID
    int newUnit = unit->itemData(idx, YonaUnits::UnitRole).toInt();

    amount->setDisplayUnit(newUnit);
}

void YonaAmountField::setDisplayUnit(int newUnit)
{
    unit->setValue(newUnit);
}

void YonaAmountField::setSingleStep(const CAmount& step)
{
