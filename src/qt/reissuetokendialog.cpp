        ...
        platformStyle(_platformStyle)
{
    ...
    ui->comboBox->lineEdit()->setPlaceholderText(tr("Select an token to reissue.."));
    ui->comboBox->lineEdit()->setToolTip(tr("Select the token you want to reissue."));
...
void ReissueTokenDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        connect(ui->groupFee, &QButtonGroup::idClicked, this, &ReissueTokenDialog::updateFeeSectionControls);
        connect(ui->groupFee, &QButtonGroup::idClicked, this, &ReissueTokenDialog::coinControlUpdateLabels);
#else
...
bool ReissueTokenDialog::eventFilter(QObject *sender, QEvent *event)
{
...
void ReissueTokenDialog::setupTokenDataView(const PlatformStyle *platformStyle)
{
    ...
    ui->labelReissueToken->setStyleSheet(STRING_LABEL_COLOR);
    ui->labelReissueToken->setFont(GUIUtil::getTopLabelFont());
...
void ReissueTokenDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& stake,
                                    const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance, const CAmount& watchOnlyStake,
                                    const CAmount& lockedBalance, const CAmount& offline)
{
...
void ReissueTokenDialog::CheckFormState()
{
    ...
    bool fReissuingRestricted = IsTokenNameAnRestricted(ui->comboBox->currentText().toStdString());
    ...
    if (!ui->addressText->text().isEmpty()) {
        if (!IsValidDestination(dest)) {
            showMessage(tr("Invalid Yona Destination Address"));
            return;
        }
    }
    ...
    if (fReissuingRestricted) {
        ...
        if (!strVerifier.empty()) {
            ...
            if (fHasQuantity && strAddress.empty()) {
                ...
                showMessage(tr("Warning: Restricted Tokens Issuance requires an address"));
                return;
            }
    ...
    if ((!ui->ipfsBox->isChecked() || (ui->ipfsBox->isChecked() && ui->ipfsText->text().isEmpty())) && ui->reissuableBox->isChecked() && ui->quantitySpinBox->value() == 0 && ui->unitSpinBox->value() == token->units && ui->lineEditVerifierString->text().isEmpty()) {
        hideMessage();
        return;
    }
...
void ReissueTokenDialog::buildUpdatedData()
{
    ...
    if (token->nHasIPFS && (!ui->ipfsBox->isChecked() || (ui->ipfsBox->isChecked() && ui->ipfsText->text().isEmpty()))) {
        QString qstr = QString::fromStdString(EncodeTokenData(token->strIPFSHash));
        if (qstr.size() == 46) {
    ...
    if (!ui->lineEditVerifierString->isHidden()) {
        if (!ui->lineEditVerifierString->text().isEmpty()) {
            ...
            if (ptokens->GetTokenVerifierStringIfExists(ui->comboBox->currentText().toStdString(), verifier)) {
    ...
    if (!verifierString.isEmpty())
        ui->updatedTokenData->append(verifierString);
...
void ReissueTokenDialog::onTokenSelected(int index)
{
    ...
    if (index > 0) {
        ...
        bool fRestrictedTokenSelected = IsTokenNameAnRestricted(qstr_name.toStdString());

        if (fRestrictedTokenSelected) {
        ...
        QString tokendatahash = "";
        if (token->nHasIPFS) {
            QString qstr = QString::fromStdString(EncodeTokenData(token->strIPFSHash));
            if (qstr.size() == 46) {
                    tokendatahash = formatBlack.arg(tr("IPFS Hash"), ":", qstr) + "\n";
            } else if (qstr.size() == 64) {
                    tokendatahash = formatBlack.arg(tr("Txid Hash"), ":", qstr) + "\n";
            }
            else {
                tokendatahash = formatBlack.arg(tr("Unknown data hash type"), ":", qstr) + "\n";
            }
        }
        ...
        if (fRestrictedTokenSelected) {
            CNullTokenTxVerifierString verifier;
            if (ptokens->GetTokenVerifierStringIfExists(qstr_name.toStdString(), verifier)) {
                verifierString = formatBlack.arg(tr("Current Verifier String"), ":", QString::fromStdString(verifier.verifier_string)) + "\n";
            } else {
                verifierString = "No verifier string found";
            }
        ...
        if (token->nHasIPFS)
            ui->currentTokenData->append(tokendatahash);
        if (fRestrictedTokenSelected)
            ui->currentTokenData->append(verifierString);
...
void ReissueTokenDialog::onVerifierStringChanged(QString verifier)
{
    buildUpdatedData();
    CheckFormState();
}
...
bool ReissueTokenDialog::checkIPFSHash(QString hash)
{
    if (!hash.isEmpty()) {
        ...
        if (!CheckEncoded(DecodeTokenData(hash.toStdString()), error)) {
            ...
            disableReissueButton();
            return false;
        } else if (DecodeTokenData(ui->ipfsText->text().toStdString()).empty()) {
            ui->ipfsText->setStyleSheet(STYLE_INVALID);
...
void ReissueTokenDialog::onReissueTokenClicked()
{
    ...
    if (IsTokenNameAnRestricted(name.toStdString())) {
    ...
    std::string strError = "";
    if (!ContextualCheckReissueToken(ptokens, reissueToken, strError, *tx.tx.get())) {
...
void ReissueTokenDialog::updateTokensList()
{
    ...
    for (auto item : tokens) {
        ...
        if (ptokens->CheckIfTokenExists(RESTRICTED_CHAR + name)) {
...
void ReissueTokenDialog::restrictedTokenSelected()
{
...
void ReissueTokenDialog::restrictedTokenUnselected()
{
...
void ReissueTokenDialog::showInvalidVerifierStringMessage(QString string)
{
    ui->lineEditVerifierString->setStyleSheet(STYLE_INVALID);
...
void ReissueTokenDialog::hideInvalidVerifierStringMessage()
{
