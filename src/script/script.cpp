...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** TOKENS START */
    case OP_AKILA_TOKEN             : return "OP_AKILA_TOKEN";
    /** TOKENS END */
...
bool CScript::IsTokenScript(int& nType, int& nScriptType, bool& fIsOwner, int& nStartingIndex) const
{
    if (this->size() > 31) {
        ...
        // OP_AKILA_TOKEN is always in the 23 index of the P2SH script if it exists
        if (nScriptType == TX_SCRIPTHASH && (*this)[23] == OP_AKILA_TOKEN) {
            // We have a potential token interacting with a P2SH
            index = SearchForYNA(*this, 25);

        }
        else if ((*this)[25] == OP_AKILA_TOKEN) { // OP_AKILA_TOKEN is always in the 25 index of the P2PKH script if it exists
...
bool CScript::IsNullToken() const
{
    return IsNullTokenTxDataScript() || IsNullGlobalRestrictionTokenTxDataScript() || IsNullTokenVerifierTxDataScript();
}

bool CScript::IsNullTokenTxDataScript() const
{
    return (this->size() > 23 &&
            (*this)[0] == OP_AKILA_TOKEN &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionTokenTxDataScript() const
{
    // 1 OP_AKILA_TOKEN followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_AKILA_TOKEN &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_AKILA_TOKEN followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_AKILA_TOKEN &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_AKILA_TOKEN) || (size() > MAX_SCRIPT_SIZE) || (GetTokenAmountFromScript(*this, nAmount) && nAmount == 0);
}
