    ...
    while (true) {
        ...
        if (pwallet->HaveAvailableCoinsForStaking(validatorVector)) {
            ...
            if (SignBlock(pblock, *pwallet, nTotalFees, pindexPrev, validatorVector)) {
                ...
                LogPrintf("ThreadStakeMiner: Successfully signed block, now trying to check it: %s\n", pblock->GetIndexHash().ToString());
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    GetMainSignals().BlockFound(pblock->GetIndexHash());
    ...
    bool fNewBlock = false;
    uint256 hash = pblock->GetIndexHash();
    if (!ProcessNewBlock(GetParams(), shared_pblock, true, &fNewBlock, hash))
        return error("CheckStake() : ProcessBlock, block not accepted");

    return true;
}

void static SoloMiner(const CChainParams& chainparams)
{
    LogPrintf("SoloMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("solo-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("SoloMiner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("SoloMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
...
int GenerateCoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&SoloMiner, boost::cref(chainparams)));
    }
