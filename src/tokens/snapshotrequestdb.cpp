...
CSnapshotRequestDBEntry::CSnapshotRequestDBEntry(
    const std::string & p_tokenName, int p_heightForSnapshot
)
{
    SetNull();
    ...
    heightAndName = std::to_string(heightForSnapshot) + tokenName;
}
...
{
    LogPrint(BCLog::REWARDS, "%s : Requesting snapshot: tokenName='%s', height=%d\n",
        __func__,
        p_tokenName.c_str(), p_heightForSnapshot);

    CSnapshotRequestDBEntry snapshotRequest(p_tokenName, p_heightForSnapshot);
...
)
{
    ...
    std::string heightAndName = std::to_string(p_heightForSnapshot) + p_tokenName;
...
bool CSnapshotRequestDB::ContainsSnapshotRequest(const std::string & p_tokenName, int p_heightForSnapshot)
{
    std::string heightAndName = std::to_string(p_heightForSnapshot) + p_tokenName;
...
bool CSnapshotRequestDB::RetrieveSnapshotRequestsForHeight(
    const std::string & p_tokenName, int p_blockHeight,
    std::set<CSnapshotRequestDBEntry> & p_tokensToSnapshot)
{
    bool tokenNameProvided = p_tokenName.length() > 0;
    if (tokenNameProvided) {
        LogPrint(BCLog::REWARDS, "%s : Looking for snapshot requests for token '%s' at height %d!\n",
            __func__,
            p_tokenName.c_str(), p_blockHeight);
    }
    else {
        ...
        if (pcursor->GetKey(key) && key.first == SNAPSHOTREQUEST_FLAG) {
            ...
            if (pcursor->GetValue(reqDbEntry)) {
                if (p_blockHeight == 0 || reqDbEntry.heightForSnapshot == p_blockHeight) {
                    //  If an token was specified, only add entries for it.
                    //  Otherwise, retrieve all entries.
                    if (!tokenNameProvided || p_tokenName == reqDbEntry.tokenName) {
                        p_tokensToSnapshot.insert(reqDbEntry);
                    }
                }
            } else {
...
void CDistributeSnapshotRequestDB::LoadAllDistributeSnapshot(std::map<uint256, CRewardSnapshot>& mapRewardSnapshots)
{
    ...
    for (auto const & item : mapRewardSnapshots) {
        LogPrint(BCLog::REWARDS, "%s : Found snapshot distribution request for Owner: %s,  Distribution: %s, Exception: %s, Height: %d, Status: %d\n",
                 __func__,
                 item.second.strOwnershipToken,
                 item.second.strDistributionToken,
