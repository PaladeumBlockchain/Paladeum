...
public:
    ...
    bool ContainsSnapshotRequest(const std::string & p_tokenName, int p_heightForSnapshot);

    // Remove a snapshot request
    bool RemoveSnapshotRequest(const std::string & p_tokenName, int p_heightForSnapshot);

    //  Retrieve all snapshot requests at the provided block height
    //      (if the block height is zero, retrieve requests at ALL HEIGHTS),
    //      limited to the specified token name (if provided)
