...
{
    // Issuing an username must contain at least 2 CTxOut(Paladeum Burn Tx, Any Number of other Outputs ..., New Token Tx)
    if (vout.size() < 2) {
...
bool CTransaction::VerifyNewToken(std::string& strError) const {
    // Issuing an Token must contain at least 3 CTxOut( Paladeum Burn Tx, Any Number of other Outputs ..., Owner Token Tx, New Token Tx)
    if (vout.size() < 3) {
        strError = "bad-txns-issue-vout-size-to-small";
        return false;
    }
...
{
    // Issuing an Token must contain at least 2 CTxOut( Paladeum Burn Tx, New Token Tx, Any Number of other Outputs...)
    if (vout.size() < 2) {
...
bool CTransaction::VerifyNewRestrictedToken(std::string& strError) const {
    // Issuing a restricted token must cointain at least 4 CTxOut(Paladeum Burn Tx, Token Creation, Root Owner Token Transfer, and CNullTokenTxVerifierString)
    if (vout.size() < 4) {
...
{
    // Reissuing an Token must contain at least 3 CTxOut ( Paladeum Burn Tx, Any Number of other Outputs ..., Reissue Token Tx, Owner Token Change Tx)
    if (vout.size() < 3) {
...
{
    // Verify 'paladeumq' is in the transaction
...
{
    // Verify 'paladeumr' is in the transaction
    CScript scriptPubKey = txOut.scriptPubKey;
...
bool CreateTokenTransaction(CWallet* pwallet, CCoinControl& coinControl, const std::vector<CNewToken> tokens, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string message, std::string* verifier_string)
{
    ...
    if (!change_address.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Paladeum address: ") + change_address);
            return false;
        }
    } else {
...
bool CreateReissueTokenTransaction(CWallet* pwallet, CCoinControl& coinControl, const CReissueToken& reissueToken, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string message, std::string* verifier_string)
{
    ...
    if (!IsValidDestinationString(address)) {
        error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Paladeum address: ") + address);
        return false;
    }
