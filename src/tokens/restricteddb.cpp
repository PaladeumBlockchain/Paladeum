...
CRestrictedDB::CRestrictedDB(size_t nCacheSize, bool fMemory, bool fWipe) : CDBWrapper(GetDataDir() / "tokens" / "restricted", nCacheSize, fMemory, fWipe) {
}

// Restricted Verifier Strings
bool CRestrictedDB::WriteVerifier(const std::string& tokenName, const std::string& verifier)
{
    return Write(std::make_pair(VERIFIER_FLAG, tokenName), verifier);
}

bool CRestrictedDB::ReadVerifier(const std::string& tokenName, std::string& verifier)
{
    return Read(std::make_pair(VERIFIER_FLAG, tokenName), verifier);
}

bool CRestrictedDB::EraseVerifier(const std::string& tokenName)
{
    return Erase(std::make_pair(VERIFIER_FLAG, tokenName));
}
...
// Address Restriction
bool CRestrictedDB::WriteRestrictedAddress(const std::string& address, const std::string& tokenName)
{
    int8_t i = 1;
    return Write(std::make_pair(RESTRICTED_ADDRESS_FLAG, std::make_pair(address, tokenName)), i);
}

bool CRestrictedDB::ReadRestrictedAddress(const std::string& address, const std::string& tokenName)
{
    int8_t i;
    return Read(std::make_pair(RESTRICTED_ADDRESS_FLAG, std::make_pair(address, tokenName)), i);
}

bool CRestrictedDB::EraseRestrictedAddress(const std::string& address, const std::string& tokenName)
{
    return Erase(std::make_pair(RESTRICTED_ADDRESS_FLAG, std::make_pair(address, tokenName)));
}

// Global Restriction
bool CRestrictedDB::WriteGlobalRestriction(const std::string& tokenName)
{
    int8_t i = 1;
    return Write(std::make_pair(GLOBAL_RESTRICTION_FLAG, tokenName), i);
}

bool CRestrictedDB::ReadGlobalRestriction(const std::string& tokenName)
{
    int8_t i;
    return Read(std::make_pair(GLOBAL_RESTRICTION_FLAG, tokenName), i);
}

bool CRestrictedDB::EraseGlobalRestriction(const std::string& tokenName)
{
    return Erase(std::make_pair(GLOBAL_RESTRICTION_FLAG, tokenName));
}
