...
#ifndef AKILACOIN_MESSAGES_H
#define AKILACOIN_MESSAGES_H
...
#endif
bool IsAddressSeen(const std::string &address); // Has this address already been sent an token before
...
public:
    ...
    std::string ToString() const {
        return strprintf("CMessage(%s, Name=%s, Message=%s, Expires=%u, Time=%u, BlockHeight=%u)", out.ToString(), strName,
                         EncodeTokenData(ipfsHash), nExpiredTime, time, nBlockHeight);
    }
...
public:
    ...
    CZMQMessage(const CMessage& message) {
        ...
        this->tokenName = message.strName;
