 ...
 * between akilad, and akila-qt and non-server tools.
*/
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), AKILA_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/AkilaProject/Akilacoin>";
    const std::string URL_WEBSITE = "<https://akilacoin.org>";
...
static void BlockNotifyCallback(bool initialSync, const CBlockIndex *pBlockIndex)
{
    ...
    if (!strCmd.empty()) {
        boost::replace_all(strCmd, "%s", pBlockIndex->GetIndexHash().GetHex());
...
/** Sanity checks
 *  Ensure that Akila is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Akila process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", AKILA_CONF_FILENAME)).string());
