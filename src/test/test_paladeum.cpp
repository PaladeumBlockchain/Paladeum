...
TestingSetup::TestingSetup(const std::string &chainName) : BasicTestingSetup(chainName)
{
    ...
    ClearDatadirCache();
    pathTemp = fs::temp_directory_path() / strprintf("test_paladeum_%lu_%i", (unsigned long) GetTime(), (int) (InsecureRandRange(100000)));
...
TestChain100Setup::CreateAndProcessBlock(const std::vector<CMutableTransaction> &txns, const CScript &scriptPubKey)
{
    ...
    while (!CheckProofOfWork(block.GetWorkHash(), block.nBits, chainparams.GetConsensus())) { ++block.nNonce64; ++block.nNonce;};
