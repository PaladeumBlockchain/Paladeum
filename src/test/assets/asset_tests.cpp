...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        BOOST_CHECK(!IsAssetNameValid("YONA", type));
        BOOST_CHECK(!IsAssetNameValid("YONA", type));
        BOOST_CHECK(!IsAssetNameValid("YONACOIN", type));

        //- Versions of YONACOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("YONA.COIN", type));
        BOOST_CHECK(IsAssetNameValid("YONA_COIN", type));
        BOOST_CHECK(IsAssetNameValid("YONASPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERYONA", type));
        BOOST_CHECK(IsAssetNameValid("YONASPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEYONA", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_YONAS", type));
        BOOST_CHECK(IsAssetNameValid("SEYONAOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("YONA", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("YONA", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
