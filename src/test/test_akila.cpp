...
TestingSetup::TestingSetup(const std::string &chainName) : BasicTestingSetup(chainName)
{
    ...
    ClearDatadirCache();
    pathTemp = fs::temp_directory_path() / strprintf("test_akila_%lu_%i", (unsigned long) GetTime(), (int) (InsecureRandRange(100000)));
...
TestChain100Setup::CreateAndProcessBlock(const std::vector<CMutableTransaction> &txns, const CScript &scriptPubKey)
{
    ...
    while (!CheckProofOfWork(block.GetBlockHash(), block.nBits, chainparams.GetConsensus())) { ++block.nNonce64; ++block.nNonce;};
