...
BOOST_FIXTURE_TEST_SUITE(blockencodings_tests, RegtestingSetup)
    ...
    static CBlock BuildBlockTestCase()
    {
        ...
        assert(!mutated);
        while (!CheckProofOfWork(block.GetWorkHash(), block.nBits, GetParams().GetConsensus())) ++block.nNonce;
        return block;
    }
...
#define SHARED_TX_OFFSET 2
    ...
    BOOST_AUTO_TEST_CASE(simple_round_trip_test)
    {
        ...
        // Do a simple ShortTxIDs RT
        {
            ...
            BOOST_CHECK_EQUAL(block.GetIndexHash().ToString(), block3.GetHash().ToString());
    ...
    BOOST_AUTO_TEST_CASE(sufficient_preforward_rt_test)
    {
        ...
        // Test with pre-forwarding coinbase + tx 2 with tx 1 in mempool
        {
            ...
            BOOST_CHECK_EQUAL(block.GetIndexHash().ToString(), block2.GetHash().ToString());
            bool mutated;
