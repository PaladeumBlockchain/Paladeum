...
BOOST_FIXTURE_TEST_SUITE(verifier_string_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(verifier_check_token_txout)
    {
        BOOST_TEST_MESSAGE("Running CheckVerifierTokenTxOut Tests");
        ...
        /// Check invalid verifier strings ///
        {
            ...
            CNullTokenTxVerifierString invalid_verifier_1("#KYC&#KYC2");
            invalid_verifier_1.ConstructTransaction(scriptPubKey);
            txout.scriptPubKey = scriptPubKey;

            BOOST_CHECK_MESSAGE(!CheckVerifierTokenTxOut(txout, error), "Verifier String didn't fail when it wasn't stripped of #");

            scriptPubKey.clear();
            CNullTokenTxVerifierString invalid_verifier_2("KYC & KYC2");
            invalid_verifier_2.ConstructTransaction(scriptPubKey);
            txout.scriptPubKey = scriptPubKey;

            BOOST_CHECK_MESSAGE(!CheckVerifierTokenTxOut(txout, error), "Verifier String didn't fail when it wasn't stripped of whitespaces");
        ...
        {
            scriptPubKey.clear();
            CNullTokenTxVerifierString valid_verifier_1("KYC&KYC2");
            valid_verifier_1.ConstructTransaction(scriptPubKey);
            txout.scriptPubKey = scriptPubKey;

            BOOST_CHECK_MESSAGE(CheckVerifierTokenTxOut(txout, error), "Verifier String failed when it was stripped correctly");
        }
    }
