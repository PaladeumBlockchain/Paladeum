...
BOOST_FIXTURE_TEST_SUITE(token_reissue_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(reissue_cache_test_ipfs)
    {
        ...
        CNewToken token1("PLDTOKEN", CAmount(100 * COIN), 8, 1, 0, "");
        ...
        CReissueToken reissue1("PLDTOKEN", CAmount(1 * COIN), 8, 1, DecodeTokenData("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        BOOST_CHECK_MESSAGE(cache.mapReissuedTokenData.count("PLDTOKEN"), "Map Reissued Token should contain the token \"PLDTOKEN\"");
        BOOST_CHECK_MESSAGE(cache.mapTokensAddressAmount.at(make_pair("PLDTOKEN", GetParams().GlobalFeeAddress())) == CAmount(101 * COIN), "Reissued amount wasn't added to the previous total");

        // Get the new token data from the cache
        CNewToken token2;
        BOOST_CHECK_MESSAGE(cache.GetTokenMetaDataIfExists("PLDTOKEN", token2), "Failed to get the token2");
        ...
        BOOST_CHECK_MESSAGE(token2.strName == "PLDTOKEN", "Token2: Token name is wrong");
        ...
        undoBlockData.emplace_back(std::make_pair("PLDTOKEN", CBlockTokenUndo{true, false, "", 0, TOKEN_UNDO_INCLUDES_VERIFIER_STRING, false, ""}));
        ...
        CNewToken token3;
        BOOST_CHECK_MESSAGE(cache.GetTokenMetaDataIfExists("PLDTOKEN", token3), "Failed to get the token3");
        ...
        BOOST_CHECK_MESSAGE(token3.strName == "PLDTOKEN", "Token3: Token name is wrong");
        ...
        BOOST_CHECK_MESSAGE(cache.mapReissuedTokenData.count("PLDTOKEN"), "Map of reissued data was removed, even though changes were made and not databased yet");
        BOOST_CHECK_MESSAGE(cache.mapTokensAddressAmount.at(make_pair("PLDTOKEN", GetParams().GlobalFeeAddress())) == CAmount(100 * COIN), "Tokens total wasn't undone when reissuance was");
    }
    ...
    BOOST_AUTO_TEST_CASE(reissue_cache_test_txid)
    {
        ...
        // Create a reissuance of the token
        CReissueToken reissue1("PLDTOKEN", CAmount(1 * COIN), 8, 1, DecodeTokenData("9c2c8e121a0139ba39bffd3ca97267bca9d4c0c1e84ac0c34a883c28e7a912ca"));
        COutPoint out(uint256S("BF50CB9A63BE0019171456252989A459A7D0A5F494735278290079D22AB704A4"), 1);
    ...
    BOOST_AUTO_TEST_CASE(reissue_isvalid_test)
    {
        ...
        CReissueToken reissue3("PLDTOKEN", CAmount(1 * COIN), 7, 1, DecodeTokenData("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CReissueToken reissue4("PLDTOKEN", CAmount(1 * COIN), -1, 1, DecodeTokenData("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CNewToken token2("PLDTOKEN2", CAmount(100 * COIN), 0, 1, 0, "");
        ...
        CReissueToken reissue5("PLDTOKEN2", CAmount(1 * COIN), 1, 1, DecodeTokenData("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CReissueToken reissue6("PLDTOKEN2", CAmount(1 * COIN), 1, 1, DecodeTokenData("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
