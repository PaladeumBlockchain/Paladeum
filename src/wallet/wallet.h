...
#ifndef PLB_WALLET_WALLET_H
#define PLB_WALLET_WALLET_H
...
public:
    ...
    bool SelectCoinsForStaking(CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, std::vector< std::string > validatorVector) const;
    ...
    void AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapTokenCoins,
                            bool fGetPLB = true, bool fOnlyTokens = false,
                            ...
                            const int& nMinDepth = 0, const int& nMaxDepth = 9999999, bool fLockedAssets = false) const;
    ...
    /**
     * Helper function that calls AvailableCoinsAll, used for fetching time locked asset utxo set 
     */
    void LockedTokens(std::map<std::string, std::vector<COutput> > &mapAssetCoins, bool fOnlySafe = true,
                         const CCoinControl *coinControl = nullptr, const CAmount &nMinimumAmount = 1,
                         const CAmount &nMaximumAmount = MAX_MONEY, const CAmount &nMinimumSumAmount = MAX_MONEY,
                         const uint64_t &nMaximumCount = 0, const int &nMinDepth = 0, const int &nMaxDepth = 9999999) const;

    /**
     * Helper function that calls AvailableCoinsAll, used to receive all coins, Tokens and PLB
     */
    ...
    void AvailableCoinsForStaking(std::vector<COutput>& vCoins, std::vector< std::string > validatorVector) const;
    uint64_t GetStakeWeight() const;
    bool HaveAvailableCoinsForStaking(std::vector< std::string > validatorVector) const;
    ...
    bool CreateCoinStake(const CKeyStore &keystore, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key, std::vector< std::string > validatorVector);
    ...
    void UpdateMyRestrictedTokens(std::string& address, std::string& token_name,
    ...
    /**
     * Wallets Restricted Token Address data added or updated.
     * @note called with lock cs_wallet held.
     */
    boost::signals2::signal<void (CWallet *wallet, std::string& address, std::string& token_name,
                                  int type, uint32_t date)> NotifyMyRestrictedTokensChanged;
