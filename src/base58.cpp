...
class CPaladeumAddressVisitor : public boost::static_visitor<bool>
{
private:
    CPaladeumAddress* addr;

public:
    explicit CPaladeumAddressVisitor(CPaladeumAddress* addrIn) : addr(addrIn) {}
...
bool CPaladeumAddress::Set(const CKeyID& id)
{
    SetData(GetParams().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CPaladeumAddress::Set(const CKeyID& id, const CKeyID& id2)
{
    SetData(GetParams().Base58Prefix(CChainParams::OFFLINE_STAKING_ADDRESS), &id, 20, &id2, 20);
    return true;
}

bool CPaladeumAddress::Set(const CScriptID& id)
{
    SetData(GetParams().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CPaladeumAddress::Set(const CTxDestination& dest)
{
    return boost::apply_visitor(CPaladeumAddressVisitor(this), dest);
}

bool CPaladeumAddress::GetSpendingAddress(CPaladeumAddress &address) const
{
...
bool CPaladeumAddress::GetStakingAddress(CPaladeumAddress &address) const
{
    if (!IsOfflineStakingAddress(GetParams()))
        return false;
...
bool CPaladeumAddress::IsValid() const
{
    return IsValid(GetParams());
}

bool CPaladeumAddress::IsValid(const CChainParams& params) const
{
...
bool CPaladeumAddress::IsOfflineStakingAddress(const CChainParams& params) const
{
    return vchVersion == params.Base58Prefix(CChainParams::OFFLINE_STAKING_ADDRESS) && vchData.size() == 40;
}

CTxDestination CPaladeumAddress::Get() const
{
    if (!IsValid())
...
bool CPaladeumAddress::GetKeyID(CKeyID& keyID) const
{
...
bool CPaladeumAddress::GetStakingKeyID(CKeyID& keyID) const
{
...
bool CPaladeumAddress::GetSpendingKeyID(CKeyID& keyID) const
{
    if (!(IsValid() && vchVersion == GetParams().Base58Prefix(CChainParams::OFFLINE_STAKING_ADDRESS)))
        return false;
    uint160 id;
...
bool CPaladeumAddress::GetIndexKey(uint160& hashBytes, int& type) const
{
    if (!IsValid()) {
        return false;
...
void CPaladeumSecret::SetKey(const CKey& vchSecret)
{
...
bool CPaladeumSecret::IsValid() const
{
...
bool CPaladeumSecret::SetString(const char* pszSecret)
{
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CPaladeumSecret::SetString(const std::string& strSecret)
{
...
{
    return CPaladeumAddress(str).Get();
}

bool IsValidDestinationString(const std::string& str, const CChainParams& params)
{
    return CPaladeumAddress(str).IsValid(params);
}

bool IsValidDestinationString(const std::string& str)
{
    return CPaladeumAddress(str).IsValid();
}
