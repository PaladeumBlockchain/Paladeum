...
The RPM spec file provided here is for Paladeum-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://paladeum.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Paladeum git repository](https://github.com/PaladeumProject/Paladeumcoin) but are not part of
...
Currently only `Source10` is used, for the example `paladeum.conf` file.
...
in the paladeum git repository. At present only an SVG version of the Paladeum
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the paladeum
...
needed when 0.12.1 is released, a proper fix is already in the Paladeum git
master branch.
...
idea to build Paladeum Core against a static version of that library compiled
...
The `paladeum.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `paladeum-server` package. At this point, most RPM based
...
The `paladeum-server` package should have SELinux support. How to properly do
...
Paladeum-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/paladeum/). He has been
packaging Paladeum for Fedora at least since 2012.
...
4. I renamed the `paladeum` package that contains the Qt GUI to `paladeum-core` as
that appears to be how the general population refers to it, in contrast to
`paladeum-xt` or `paladeum-classic`. I wanted to make sure the general population
