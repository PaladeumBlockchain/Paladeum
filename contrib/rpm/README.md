...
The RPM spec file provided here is for Akila-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://akila.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Akila git repository](https://github.com/AkilaProject/Akilacoin) but are not part of
...
Currently only `Source10` is used, for the example `akila.conf` file.
...
in the akila git repository. At present only an SVG version of the Akila
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the akila
...
needed when 0.12.1 is released, a proper fix is already in the Akila git
master branch.
...
idea to build Akila Core against a static version of that library compiled
...
The `akila.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `akila-server` package. At this point, most RPM based
...
The `akila-server` package should have SELinux support. How to properly do
...
Akila-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/akila/). He has been
packaging Akila for Fedora at least since 2012.
...
4. I renamed the `akila` package that contains the Qt GUI to `akila-core` as
that appears to be how the general population refers to it, in contrast to
`akila-xt` or `akila-classic`. I wanted to make sure the general population
