...
The RPM spec file provided here is for Yona-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://yona.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Yona git repository](https://github.com/YonaProject/Yonacoin) but are not part of
...
Currently only `Source10` is used, for the example `yona.conf` file.
...
in the yona git repository. At present only an SVG version of the Yona
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the yona
...
needed when 0.12.1 is released, a proper fix is already in the Yona git
master branch.
...
idea to build Yona Core against a static version of that library compiled
...
The `yona.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `yona-server` package. At this point, most RPM based
...
The `yona-server` package should have SELinux support. How to properly do
...
Yona-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/yona/). He has been
packaging Yona for Fedora at least since 2012.
...
4. I renamed the `yona` package that contains the Qt GUI to `yona-core` as
that appears to be how the general population refers to it, in contrast to
`yona-xt` or `yona-classic`. I wanted to make sure the general population
