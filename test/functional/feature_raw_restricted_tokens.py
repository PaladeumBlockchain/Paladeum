...
def get_tx_issue_hex(node, to_address, token_name, token_quantity=1000, verifier_string="true", units=0, reissuable=1, has_ipfs=0, ipfs_hash="", owner_change_address=""):
    ...
    yona_unspent = next(u for u in node.listunspent() if u['amount'] > BURN_AMOUNTS['issue_restricted'])
    yona_inputs = [{k: yona_unspent[k] for k in ['txid', 'vout']}]
    ...
    outputs = {
        ...
        change_address: truncate(float(yona_unspent['amount']) - BURN_AMOUNTS['issue_restricted'] - FEE_AMOUNT),
        to_address: {
            'issue_restricted': {
    ...
    tx_issue = node.createrawtransaction(yona_inputs + owner_inputs, outputs)
...
def get_tx_reissue_hex(node, to_address, token_name, token_quantity, reissuable=1, verifier_string="", ipfs_hash="", owner_change_address=""):
    ...
    yona_unspent = next(u for u in node.listunspent() if u['amount'] > BURN_AMOUNTS['reissue_restricted'])
    yona_inputs = [{k: yona_unspent[k] for k in ['txid', 'vout']}]
    ...
    outputs = {
        ...
        change_address: truncate(float(yona_unspent['amount']) - BURN_AMOUNTS['reissue_restricted'] - FEE_AMOUNT),
        to_address: {
            'reissue_restricted': {
                'token_name': token_name,
                'token_quantity': token_quantity,
...
def get_tx_issue_qualifier_hex(node, to_address, token_name, token_quantity=1, has_ipfs=0, ipfs_hash="", root_change_address="", change_qty=1):
    ...
    yona_unspent = next(u for u in node.listunspent() if u['amount'] > BURN_AMOUNTS['issue_qualifier'])
    yona_inputs = [{k: yona_unspent[k] for k in ['txid', 'vout']}]
    ...
    outputs = {
        ...
        change_address: truncate(float(yona_unspent['amount']) - burn_amount - FEE_AMOUNT),
        to_address: {
            'issue_qualifier': {
                'token_name': token_name,
                'token_quantity': token_quantity,
    ...
    tx_issue = node.createrawtransaction(yona_inputs + root_inputs, outputs)
    tx_issue_signed = node.signrawtransaction(tx_issue)
    tx_issue_hex = tx_issue_signed['hex']
    return tx_issue_hex
...
def get_tx_transfer_hex(node, to_address, token_name, token_quantity):
    ...
    yona_unspent = next(u for u in node.listunspent() if u['amount'] > FEE_AMOUNT)
    yona_inputs = [{k: yona_unspent[k] for k in ['txid', 'vout']}]
    ...
    outputs = {
        change_address: truncate(float(yona_unspent['amount']) - FEE_AMOUNT),
        to_address: {
            'transfer': {
    ...
    tx_transfer = node.createrawtransaction(yona_inputs + token_inputs, outputs)
...
def get_tx_tag_address_hex(node, op, qualifier_name, tag_addresses, qualifier_change_address, change_qty=1):
    ...
    yona_unspent = next(u for u in node.listunspent() if u['amount'] > burn_amount)
    yona_inputs = [{k: yona_unspent[k] for k in ['txid', 'vout']}]
    ...
    tx_tag = node.createrawtransaction(yona_inputs + qualifier_inputs, outputs)
...
def get_tx_freeze_address_hex(node, op, token_name, freeze_addresses, owner_change_address):
    ...
    tx_freeze = node.createrawtransaction(yona_inputs + owner_inputs, outputs)
...
class RawRestrictedTokensTest(YonaTestFramework):
