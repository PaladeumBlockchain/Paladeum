...
class TokenReorgTest(YonaTestFramework):
    ...
    def activate_tokens(self):
        self.log.info("Generating YONA and activating tokens...")
    ...
    def issue_reorg_test(self):
        ...
        token_name = "DOUBLE_TROUBLE"

        # Issue token on chain 1
        n0.issue(token_name)
        n0.generate(15)

        # Issue token on chain 2
        n1.generate(5)
        n1.issue(token_name)
        n1.generate(5)
        ...
        # They should each on their chains the token DOUBLE_TROUBLE and DOUBLE_TROUBLE!
        assert_equal(True, n0.listmytokens() == n1.listmytokens())
        ...
        # self.log.info(n0.listmytokens())
        # self.log.info(n1.listmytokens())
        ...
        # Verify that node1 reorged to the node0 chain and that node0 has the token and not node1
        ...
        # Issue second token on chain 1 and mine 5 blocks
        n0.issue(token_name + '2')
        n0.generate(5)

        # Mine 5 blocks, issue second token on chain 2, mine 5 more blocks, giving node1 more chain weight
        n1.generate(5)
        n1.issue(token_name + '2')
        n1.generate(5)

        node_1_hash = n1.getbestblockhash()
        node_1_height = n1.getblockcount()
        assert_equal(True, n0.getbestblockhash() is not node_1_hash)
        assert_equal(True, n0.listmytokens('DOUBLE_TROUBLE2*') == n1.listmytokens('DOUBLE_TROUBLE2*'))

        # Connect the nodes together, and force a reorg to occur
        connect_all_nodes_bi(self.nodes, True)

        # Verify that node0 reorged to the node1 chain and that node1 has the token and not node0
