...
class RawAssetTransactionsTest(YonaTestFramework):
    ...
    def issue_tampering_test(self):
        ...
        for n in range(0, len(tx.vout)):
            ...
            if yonao in bytes_to_hex_str(out.scriptPubKey):
                ...
                asset_script_hex = owner_script_hex[owner_script_hex.index(yonao) + len(yonao):-len(op_drop)]
        ...
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        assert_raises_rpc_error(-26, "bad-txns-op-yona-asset-not-in-right-script-location",
                                n0.sendrawtransaction, tx_bad_issue_signed)
    ...
    def issue_sub_multiple_outputs_test(self):
        ...
        for n in range(0, len(tx.vout)):
            ...
            if yonat in bytes_to_hex_str(out.scriptPubKey):
                ...
                asset_script_hex = transfer_script_hex[transfer_script_hex.index(yonat) + len(yonat):-len(op_drop)]
                f = BytesIO(hex_str_to_bytes(asset_script_hex))
                transfer = CScriptTransfer()
                transfer.deserialize(f)
                transfer.amount = 0
