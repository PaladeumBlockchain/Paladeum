...
# TestNode: A peer we use to send messages to akilad, and store responses.
...
class CompactBlocksTest(AkilaTestFramework):
    ...
    # This test actually causes akilad to (reasonably!) disconnect us, so do this last.
    ...
    # Test that akilad requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for akilad to be smart enough to know we're
    ...
    def test_getblocktxn_handler(node, test_node, version):
        # akilad will not send blocktxn responses for blocks whose height is
