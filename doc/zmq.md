...
The Akila Core daemon can be configured to act as a trusted "border
router", implementing the akila wire protocol and relay, making
...
The ZeroMQ feature in Akila Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building akilad:
...
For instance:
    ...
    $ akilad -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/akilad.tx.raw
...
These options can also be provided in akila.conf.
...
From the perspective of akilad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into akilad directly. Furthermore, no information is
...
using. Akilad appends an up-counting sequence number to each
