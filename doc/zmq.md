...
The Paladeum Core daemon can be configured to act as a trusted "border
router", implementing the paladeum wire protocol and relay, making
...
The ZeroMQ feature in Paladeum Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building paladeumd:
...
For instance:
    ...
    $ paladeumd -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/paladeumd.tx.raw
...
These options can also be provided in paladeum.conf.
...
From the perspective of paladeumd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into paladeumd directly. Furthermore, no information is
...
using. Paladeumd appends an up-counting sequence number to each
