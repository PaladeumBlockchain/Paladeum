...
The Yona Core daemon can be configured to act as a trusted "border
router", implementing the yona wire protocol and relay, making
...
The ZeroMQ feature in Yona Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building yonad:
...
For instance:
    ...
    $ yonad -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/yonad.tx.raw
...
These options can also be provided in yona.conf.
...
From the perspective of yonad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into yonad directly. Furthermore, no information is
...
using. Yonad appends an up-counting sequence number to each
